...
export class RoomScene {
  ...
  private floorMaterials: Map<FloorMaterialType, FloorMaterialData> = new Map();
  ...
  private bindEvents(): void {
    ...
    eventBus.on(EVENTS.ROOM_CHANGE, (data: { roomType: RoomType }) => {
      this.loadRoom(data.roomType);
    });

    eventBus.on(EVENTS.WALL_COLOR_CHANGE, (data: { color: string }) => {
      this.updateWallColor(data.color);
    });

    eventBus.on(EVENTS.FLOOR_MATERIAL_CHANGE, (data: { material: FloorMaterialType }) => {
      this.updateFloorMaterial(data.material);
    });

    eventBus.on(EVENTS.FURNITURE_SIZE_CHANGE, (data: { id: string; size: any }) => {
      this.updateFurnitureSize(data.id, data.size);
    });

    eventBus.on(EVENTS.FURNITURE_POSITION_CHANGE, (data: { id: string; position: any }) => {
      this.updateFurniturePosition(data.id, data.position);
    });

    eventBus.on(EVENTS.LIGHT_DIRECTION_CHANGE, (data: { angle: number }) => {
    ...
    eventBus.on(EVENTS.SNAPSHOT_LOAD, (data: SnapshotData) => {
      this.loadSnapshot(data);
    });
  }

  private loadRoom(roomType: RoomType): void {
  ...
  private animateRoomTransition(callback: () => void): void {
    const duration = 500;
  ...
  private createRoom(template: any): void {
    ...
    template.furniture.forEach((furnitureData: FurnitureData) => {
  ...
  private createFurniture(data: FurnitureData): void {
  ...
  private createBox(w: number, h: number, d: number, color: THREE.Color): THREE.Mesh {
  ...
  private createSofa(w: number, d: number, h: number, color: THREE.Color): THREE.Mesh {
    const group = new THREE.Group();
  ...
  private createBed(w: number, d: number, h: number, color: THREE.Color): THREE.Mesh {
    const group = new THREE.Group();
  ...
  private createChair(w: number, d: number, h: number, color: THREE.Color): THREE.Mesh {
    const group = new THREE.Group();
  ...
  private createLamp(w: number, d: number, h: number, color: THREE.Color): THREE.Mesh {
    const group = new THREE.Group();
  ...
  private createCabinet(w: number, d: number, h: number, color: THREE.Color): THREE.Mesh {
    const group = new THREE.Group();
  ...
  private createWardrobe(w: number, d: number, h: number, color: THREE.Color): THREE.Mesh {
    const group = new THREE.Group();
  ...
  private createFridge(w: number, d: number, h: number, color: THREE.Color): THREE.Mesh {
    const group = new THREE.Group();
  ...
  private groupToSingleMesh(group: THREE.Group, defaultColor: THREE.Color): THREE.Mesh {
    let mainMesh: THREE.Mesh | null = null;
    group.traverse((obj) => {
      if ((obj as THREE.Mesh).isMesh && !mainMesh) {
        mainMesh = obj as THREE.Mesh;
      }
    });
    if (mainMesh) {
      return mainMesh;
  ...
  private updateWallColor(color: string): void {
  ...
  private updateFloorMaterial(materialType: FloorMaterialType): void {
    if (!this.floor) return;
    if (this.currentParams.floorMaterial === materialType) return;

    const newMaterialData = this.floorMaterials.get(materialType);
    if (!newMaterialData) return;

    this.animateFloorTexture(materialType);
    this.currentParams.floorMaterial = materialType;
  }

  private animateFloorTexture(targetType: FloorMaterialType): void {
    if (!this.floor) return;

    const currentMat = this.floor.material as THREE.MeshStandardMaterial;
    const targetMaterialData = this.floorMaterials.get(targetType);
    if (!targetMaterialData) return;
  ...
  private updateAmbientLightColor(): void {
  ...
  private setLightDirection(angleDeg: number): void {
  ...
  private onCanvasClick(event: MouseEvent): void {
    ...
    this.furnitureMap.forEach((furniture) => {
      furniture.group.traverse((obj) => {
        if ((obj as THREE.Mesh).isMesh) {
          furnitureMeshes.push(obj as THREE.Mesh);
        }
      });
    });
  ...
  private updateFurnitureSize(id: string, size: { width: number; depth: number; height: number }): void {
    const furniture = this.furnitureMap.get(id);
    if (!furniture) return;
  ...
  private updateFurniturePosition(id: string, position: { x: number; z: number }): void {
    const furniture = this.furnitureMap.get(id);
    if (!furniture) return;
  ...
  private recreateFurniture(id: string): void {
    const furniture = this.furnitureMap.get(id);
    if (!furniture) return;
  ...
  private getCurrentFurnitureData(): FurnitureData[] {
  ...
  private animateSnapshotTransition(snapshot: SnapshotData, callback: () => void): void {
    const duration = 1000;
    const startTime = performance.now();
    ...
    const animate = (time: number) => {
      ...
      this.setLightDirection(
        startParams.lightAngle + (endParams.lightAngle - startParams.lightAngle) * eased
      );
