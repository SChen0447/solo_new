...
const genTempId = (): string => Math.random().toString(36).slice(2, 10);
...
export default function App() {
  ...
  const onListScroll = () => {
    ...
    const end = Math.min(filteredSurveys.length, Math.ceil((scrollTop + viewportHeight) / ITEM_HEIGHT) + buffer);
  ...
  const updateQuestion = (tempId: string, patch: Partial<EditableQuestion>) => {
    setQuestions((q) =>
      q.map((x) => (x._tempId === tempId ? { ...x, ...patch } : x))
    );
  };
  ...
  const loadStats = async (id: string) => {
    try {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        try {
          await submitResponse(id, randomAnswers);
        } catch {
          break;
        }
      }
      ...
      return (
        <div style={{ ...baseStyles, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={cardStyle}>
            ...
            <RippleButton onClick={goHome} style={primaryBtnStyle}>
              返回首页
            </RippleButton>
          </div>
        </div>
      ...
      <div style={{ ...baseStyles, padding: '32px 16px' }}>
        ...
        <div style={{ maxWidth: 720, margin: '0 auto' }}>
          <div style={{ ...cardStyle, background: `linear-gradient(135deg, ${PRIMARY_COLOR}, ${PRIMARY_DARK})`, color: 'white', marginBottom: 24 }}>
            <h1 style={{ margin: 0, fontSize: 26 }}>{fillSurvey!.title}</h1>
            <div style={{ marginTop: 12, opacity: 0.9, fontSize: 14 }}>
              共 {fillSurvey!.questions.length} 个问题 · 请认真填写
            </div>
          </div>

          {fillSurvey!.questions.map((q, idx) => (
            <div key={q.id} style={{ ...cardStyle, marginBottom: 16, animation: `fadeInUp 0.4s ease-out ${idx * 0.05}s both` }}>
              ...
              {q.type === 'single' && q.options && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10, paddingLeft: 36 }}>
                  {q.options.map((opt, i) => {
                    const checked = fillAnswers[q.id] === opt;
                    return (
                      <label key={i} style={{
                        display: 'flex',
                        alignItems: 'center',
                        ...
                        <input
                          ...
                          style={{ display: 'none' }}
                          checked={checked}
                          onChange={() => setFillAnswers((a) => ({ ...a, [q.id]: opt }))}
                        />
              ...
              {q.type === 'multiple' && q.options && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10, paddingLeft: 36 }}>
                  {q.options.map((opt, i) => {
                    const cur = (fillAnswers[q.id] as string[]) || [];
                        ...
                        <input
                          ...
                          onChange={() => {
                            setFillAnswers((a) => {
                              ...
                              if (checked) {
                                return { ...a, [q.id]: arr.filter((x) => x !== opt) };
                              }
                              return { ...a, [q.id]: [...arr, opt] };
                            });
                          }}
                        />
                        <span style={{ color: '#333' }}>{opt}</span>
              ...
              {q.type === 'text' && (
                <div style={{ paddingLeft: 36 }}>
                  <textarea
                    rows={4}
                    value={(fillAnswers[q.id] as string) || ''}
                    onChange={(e) => setFillAnswers((a) => ({ ...a, [q.id]: e.target.value }))}
                    placeholder="请输入您的回答..."
                    style={{
                      width: '100%',
          ...
          <div style={{ display: 'flex', gap: 12, marginBottom: 32 }}>
            <RippleButton onClick={handleFillSubmit} style={{ ...primaryBtnStyle, flex: 1, fontSize: 16, padding: '14px 24px' }}>
              提交问卷
            </RippleButton>
          </div>
        </div>
    ...
    <div style={baseStyles}>
      ...
      <div className="main-layout" style={{ display: 'flex', minHeight: '100vh' }}>
        ...
        <div className="content-area" style={{
          ...
          marginLeft: view === 'create' ? 0 : (sidebarOpen ? 0 : 0),
...
function MobileTopBar({ onToggle, total }: { onToggle: () => void; open: boolean; total: number }) {
  return (
...
function Sidebar({
  ...
  const mobileSidebarStyle = window.innerWidth <= 768 ? {
  ...
  return (
    <aside className={`sidebar ${open ? '' : 'closed'}`} style={{ ...sidebarStyle, ...mobileSidebarStyle }}>
      <div style={{
      ...
      >
        {loading ? (
          <div style={{ padding: 30, textAlign: 'center', fontSize: 14, opacity: 0.6 }}>加载中...</div>
        ) : surveys.length === 0 ? (
          <div style={{ padding: 30, textAlign: 'center', fontSize: 13, opacity: 0.5 }}>
            {searchTerm ? '未找到匹配的问卷' : '暂无问卷'}
          </div>
        ) : (
          <div style={{ position: 'relative', height: surveys.length * itemHeight }}>
            {surveys.slice(visibleRange.start, visibleRange.end).map((s, i) => {
...
function ListView({ total, onCreate, loading }: { total: number; onCreate: () => void; loading: boolean }) {
  return (
        ...
        >
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 14 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, color: '#666', cursor: 'pointer' }}>
                  <input
                    type="checkbox"
                    checked={q.required}
                    onChange={(e) => onUpdateQuestion(q._tempId, { required: e.target.checked })}
              ...
              <input
                value={q.title}
                onChange={(e) => onUpdateQuestion(q._tempId, { title: e.target.value })}
                placeholder={`请输入第 ${idx + 1} 题的问题标题`}
                style={{
                  width: '100%',
                  boxSizing: 'border-box',
                  padding: '10px 14px',
            ...
            <RippleButton
              onClick={() => onRemoveQuestion(q._tempId)}
              style={{
                width: 32,
                height: 32,
                borderRadius: 8,
            ...
            <div style={{ paddingLeft: 48, display: 'flex', flexDirection: 'column', gap: 8 }}>
              {q.options.map((opt, optIdx) => (
                <div key={optIdx} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <input
                    value={opt}
                    onChange={(e) => onUpdateOption(q._tempId, optIdx, e.target.value)}
                  ...
                  />
                  <RippleButton
                    onClick={() => onRemoveOption(q._tempId, optIdx)}
                    style={{
                      width: 28, height: 28,
                      borderRadius: 6,
                      border: 'none',
          ...
          <div style={{ marginTop: 12, paddingLeft: 48, fontSize: 11, color: '#BBB' }}>
            ↕ 拖拽此卡片可调整题目顺序
          </div>
        </div>
      ))}
