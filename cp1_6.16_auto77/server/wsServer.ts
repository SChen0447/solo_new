...
import { encodeMIDI, Track, Note } from './midiEncoder.js';
...
function broadcastRoom(room: Room, senderWs?: WebSocket) {
...
function sendUserState(ws: WebSocket, room: Room) {
  const userList = Array.from(room.users.values());
  const notesList = Array.from(room.notes.values());
  ...
  ws.on('message', (data) => {
    try {
      ...
      switch (message.type) {
        ...
        case 'addTrack': {
          if (!currentRoom) break;
          const track: Track = message.data.track;
        ...
        case 'updateTrack': {
          if (!currentRoom) break;
          const track = message.data.track as Track;
